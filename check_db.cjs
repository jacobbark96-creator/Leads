...
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://pznqrbfgrvfmkdprifst.supabase.co',
...
async function check() {
  const { data, error } = await supabase.from('contractors').select('last_dialed_at').limit(1);
  console.log(error);
}
check();
