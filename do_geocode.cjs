...
const supabaseUrl = env['NEXT_PUBLIC_SUPABASE_URL'] || 'https://pznqrbfgrvfmkdprifst.supabase.co';
const supabaseKey = env['SUPABASE_SERVICE_ROLE_KEY'] || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB6bnFyYmZncnZmbWtkcHJpZnN0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3Njc2NDE0MywiZXhwIjoyMDkyMzQwMTQzfQ.okirNkFN20GeLmTaSpKymcr-VAZhvpmlGRubGUkllO0';
...
async function run() {
  ...
  const { data: contractors, error } = await supabase.from('contractors').select('id, csv_data, clients(address)').is('latitude', null).limit(100);
