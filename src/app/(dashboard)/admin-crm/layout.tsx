...
import { Users, Tags, Ticket, TrendingUp, Activity, Menu, X, LayoutDashboard, Database, HelpCircle, LogOut, Settings, BarChart2, Bell, MessageSquare, ChevronDown, Store, Briefcase, Home, Sparkles, Mail } from 'lucide-react';
      ...
      <div className="min-h-screen bg-[#F9FAFB] flex font-sans text-gray-900">
        ...
        <aside className="fixed inset-y-0 left-0 w-56 bg-white border-r border-gray-200 flex flex-col z-20 hidden md:flex">
          ...
          <div className="px-5 py-3 shrink-0">
            <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Admin CRM</h2>
          </div>
              ...
              <div className="relative">
                ...
                {crmDropdownOpen && (
                  <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1">
                    ...
                    <Link href="/contractor-crm" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 font-medium"><Briefcase className="w-4 h-4" /> Contractor CRM</Link>
                    {(profile?.role === 'admin' || profile?.role === 'super_admin') && (
                      <Link href="/admin-crm" className="flex items-center gap-2 px-4 py-2 text-sm text-blue-700 bg-blue-50 font-medium"><Settings className="w-4 h-4" /> Admin CRM</Link>
                    )}
                  </div>
                )}
              </div>
