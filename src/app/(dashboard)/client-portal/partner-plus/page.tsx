    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="mb-6 flex items-center gap-3 shrink-0">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center">
            Partner<span className="text-[12px] -mt-3 ml-[1px]">＋</span> Pipeline
          </h1>
          <p className="text-sm text-slate-500 font-medium">Manage your assigned sales pipeline and installations.</p>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
        {columns.map(col => (
          <div 
            key={col.id}
            className="flex-1 min-w-[300px] max-w-[350px] flex flex-col bg-slate-100/50 rounded-2xl border border-slate-200/60 p-4"
            onDragOver={(e) => handleDragOver(e, col.id)}
            onDrop={(e) => handleDrop(e, col.id)}
          >
            <div className="flex items-center justify-between mb-4 px-1">
              <h3 className="font-bold text-slate-800 uppercase tracking-wider text-sm flex items-center gap-2">
                <div className={`w-2.5 h-2.5 rounded-full bg-${col.color}-500 shadow-[0_0_8px_rgba(0,0,0,0.2)]`} style={{ boxShadow: `0 0 10px var(--tw-colors-${col.color}-400)` }} />
                {col.label}
              </h3>
              <span className="bg-white text-slate-600 text-xs font-bold px-2.5 py-1 rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
              {leads.filter(l => l.partner_plus_status === col.id).map(lead => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-black text-slate-900 text-sm">{lead.company || lead.name}</h4>
                      <p className="text-xs text-slate-500 font-medium">{lead.location}</p>
                    </div>
                  </div>
              ...
              {leads.filter(l => l.partner_plus_status === col.id).length === 0 && (
                <div className="h-24 flex items-center justify-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">No Leads</span>
                </div>
              )}
            </div>
          </div>
