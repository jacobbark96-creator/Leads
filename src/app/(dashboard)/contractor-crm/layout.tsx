...
import { Upload, Users, CheckCircle, UserPlus, Menu, X, LayoutDashboard, Database, HelpCircle, LogOut, Settings, BarChart2, Bell, MessageSquare, ChevronDown, Store, Home } from 'lucide-react';
      ...
      <div className="min-h-screen bg-[#F9FAFB] flex font-sans text-gray-900">
        ...
        <aside className="fixed inset-y-0 left-0 w-56 bg-white border-r border-gray-200 flex flex-col z-20 hidden md:flex">
          ...
          <div className="px-5 py-3 shrink-0">
            <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Contractor CRM</h2>
          </div>
              ...
              <div className="relative">
                ...
                {crmDropdownOpen && (
                  <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1">
                    <Link href="/sales-crm" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 font-medium"><Database className="w-4 h-4" /> Sales CRM</Link>
                    <Link href="/contractor-crm" className="flex items-center gap-2 px-4 py-2 text-sm text-blue-700 bg-blue-50 font-medium"><Users className="w-4 h-4" /> Contractor CRM</Link>
            ...
            <div className="flex items-center gap-3">
              {profile?.role && ['admin', 'super_admin', 'rep'].includes(profile.role) && (
                <button
                  ...
                  className="inline-flex items-center px-2.5 py-1.5 border border-gray-200 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  <UserPlus className="w-3 h-3 mr-1.5 text-gray-400" />
