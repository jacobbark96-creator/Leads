...
function MarketplaceAdminContent() {
  ...
  const toggleMarketStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      // If we are pausing it, ask if they want to mark it as sold
      if (currentStatus) {
        markAsSold = window.confirm('Do you want to mark this lead as sold so it appears in the "Recently Sold" carousel on the marketplace?');
      }

      const { error } = await supabase
        .from('leads')
        .update({ 
          is_marketed: !currentStatus,
          ...(markAsSold && { marked_as_sold: true, purchase_date: new Date().toISOString() })
        })
        .eq('id', id);

      if (error) throw error;
      
      toast.success(`Lead ${!currentStatus ? 'published to' : 'paused from'} marketplace${markAsSold ? ' and marked as sold' : ''}`);
