...
export default function IntranetLayout({ children }: { children: React.ReactNode }) {
  ...
  const isTracker = pathname === '/intranet/tracker';

  const sidebarLinks = [
    { name: 'Leaderboard', path: '/intranet', icon: Award, id: 'intranet/leaderboard' },
    { name: 'Pricing Matrix', path: '/intranet/pricing', icon: DollarSign, id: 'intranet/pricing' },
      ...
      <div className="min-h-screen bg-[#f4f7f6] font-sans text-gray-900 selection:bg-emerald-500/30 flex flex-col">
        ...
        >
          ...
          <main className="flex-1 min-w-0 flex flex-col gap-6">
            ...
            {/* Page Content */}
            <div className={`bg-white rounded-3xl shadow-sm border border-gray-100 ${isTracker ? 'p-0 border-none bg-transparent shadow-none' : 'p-6'}`}>
              {children}
            </div>
          </main>
