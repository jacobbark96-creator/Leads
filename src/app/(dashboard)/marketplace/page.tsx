...
export default function MarketplacePage() {
  ...
  const fetchMarketplaceLeads = async (pageNumber: number, isInitial: boolean) => {
    try {
      ...
      // Fetch recently sold leads using the bypass-RLS RPC (visible to everyone on the marketplace)
      const { data: soldData } = await supabase.rpc('get_recently_sold_leads');
      if (soldData) setSoldLeads(soldData as Lead[]);
  ...
  useEffect(() => {
    ...
    if (loading) {
      timeout = setTimeout(() => {
        console.warn('Marketplace loading timed out after 10s');
        setLoading(false);
      }, 10000);
    }
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-900 truncate pl-2">
                      {lead.monthly_spend ? `£${Number(lead.monthly_spend).toLocaleString()}/mo` : ''}
                    </span>
                  </div>
                  {/*
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500">Est. System Size:</span>
                    <span className="font-semibold text-gray-900 truncate pl-2">{lead.est_system_size || ''}</span>
                  </div>
                  */}
