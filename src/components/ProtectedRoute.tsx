...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, allowedRoles }) => {
  ...
  const [profileCheckLoading, setProfileCheckLoading] = useState(() => {
    // If we already know they are not a client, skip the loading state
    if (initialized && profile && profile.role !== 'client') return false;
    return true;
  });
  ...
  useEffect(() => {
    ...
    if (profileCheckLoading) {
      fallbackTimeout = setTimeout(() => {
        console.warn('ProtectedRoute profile check fallback timeout hit');
        setProfileCheckLoading(false);
      }, 8000);
    }
