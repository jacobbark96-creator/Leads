  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        .from('users')
        .update({ trade_limit_setting: limit })
        .eq('id', userId);

      if (error) throw error;
      
      // Update the profile in the store immediately so other components see it
      if (profile) {
        setProfile({ ...profile, trade_limit_setting: limit });
      }
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full animate-in zoom-in duration-300">
            ...
            <div className="space-y-8">
              <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl flex gap-3">
                ...
                <p className="text-sm text-blue-700 leading-relaxed">
                  Your approved trade amount is <strong>£{approvedAmount.toLocaleString()}</strong>. You can adjust your active credit limit below.
                </p>
              </div>
