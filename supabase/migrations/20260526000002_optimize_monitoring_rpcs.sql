-- Optimize Monitoring RPCs for better performance
-- Update get_client_monitoring_stats with better filtering
...
BEGIN
    ...
    WITH session_stats AS (
        ...
        WHERE cs.user_id IN (SELECT u.id FROM users u WHERE u.role IN ('client', 'rep'))
        GROUP BY cs.user_id
    ),
    lead_views AS (
        SELECT 
        ...
        AND le.user_id IN (SELECT u.id FROM users u WHERE u.role IN ('client', 'rep'))
        GROUP BY le.user_id
    ),
    top_leads AS (
        SELECT 
...
-- Update get_lead_monitoring_stats with better filtering
...
BEGIN
    ...
    WITH event_stats AS (
        ...
        FROM lead_events le
        WHERE le.lead_id IN (SELECT id FROM leads WHERE is_marketed = true)
        GROUP BY le.lead_id
    ),
    viewer_stats AS (
        SELECT 
        ...
        FROM lead_events le
        WHERE le.event_type = 'view'
        AND le.lead_id IN (SELECT id FROM leads WHERE is_marketed = true)
