-- Drop all existing versions of purchase_lead to resolve overloading ambiguity
DROP FUNCTION IF EXISTS public.purchase_lead(UUID, UUID, VARCHAR, NUMERIC, NUMERIC);
DROP FUNCTION IF EXISTS public.purchase_lead(UUID, UUID, VARCHAR, NUMERIC, NUMERIC, BOOLEAN);

-- Re-create the latest version of purchase_lead
...
BEGIN
  ...
  IF p_use_trade_account THEN
    ...
    IF (COALESCE(v_user.current_trade_usage, 0) + p_price_paid) > COALESCE(v_user.trade_limit_setting, 0) THEN
      RAISE EXCEPTION 'Trade limit exceeded. Your remaining limit is £%', (COALESCE(v_user.trade_limit_setting, 0) - COALESCE(v_user.current_trade_usage, 0));
    END IF;
