-- Fix: Show user's name instead of parent's company name for child accounts in monitoring
...
BEGIN
    ...
    SELECT 
        u.id as user_id,
        CASE 
            WHEN u.parent_id IS NOT NULL THEN u.name || ' (' || COALESCE(c.company_name, '') || ')'
            ELSE COALESCE(c.company_name, u.name)
    ...
    FROM users u
    LEFT JOIN clients c ON c.user_id = COALESCE(u.parent_id, u.id)
